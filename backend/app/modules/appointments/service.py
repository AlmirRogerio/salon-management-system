...
class AppointmentsService:
    ...
    async def get_detail(
        ...
        appointment = await self._get_owned_or_404(appointment_id, customer_id)
    ...
    async def reschedule(
        ...
        if appointment.status in (
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Nao e possivel alterar um agendamento finalizado ou cancelado.",
            )
        ...
        if new_date <= now():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="A nova data do agendamento deve ser no futuro.",
            )
    ...
    async def _get_owned_or_404(
        ...
        appointment = await self._repository.get_owned(
